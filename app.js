...
// ===== GIF Worker Inlining for file:// protocol support =====
...
function getWorkerURL() {
    if (!gifWorkerURL) {
        const blob = new Blob([GIF_WORKER_CODE], { type: 'text/javascript' });
...
async function generatePersonalizedGif() {
    ...
    try {
        ...
        // Use Logical Screen Descriptor dimensions for the full GIF size
        const { width, height } = gif.lsd;

        const encoder = new window.GIF({
            workers: 2,
            quality: 1, // Best quality (slowest, but least distortion)
            width: width,
            height: height,
            workerScript: getWorkerURL(),
            dither: 'FloydSteinberg' // Smoother color transitions
        });
        ...
        for (let i = 0; i < frames.length; i++) {
            ...
            // Handle disposal methods for correct frame assembly
            if (f.disposalType === 3 && prevData === null) {
                prevData = frameCtx.getImageData(0, 0, width, height);
            }
            
            // In gifuct-js, when buildFrames is true, f.patch is RGBA but still only the size of the patch
            ...
            // Post-draw disposal
            if (f.disposalType === 2) {
                frameCtx.clearRect(f.dims.left, f.dims.top, f.dims.width, f.dims.height);
            } else if (f.disposalType === 3 && prevData) {
                frameCtx.putImageData(prevData, 0, 0);
            }
